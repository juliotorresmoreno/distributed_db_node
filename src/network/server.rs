...
impl Server {
    ...
    pub async fn handle_message(&mut self, message: &Message) {
        match message.header.message_type {
            ...
            MESSAGE_TYPE_USE_DATABASE => handle_use_database(self, &message).await,
            ...
            MESSAGE_TYPE_RENAME_TABLE => handle_rename_table(self, &message).await,
            MESSAGE_TYPE_TRUNCATE_TABLE => handle_truncate_table(self, &message).await,
            MESSAGE_TYPE_SHOW_TABLES => handle_show_tables(self, &message).await,
            MESSAGE_TYPE_DESCRIBE_TABLE => handle_describe_table(self, &message).await,
            ...
            MESSAGE_TYPE_SHOW_INDEXES => handle_show_indexes(self, &message).await,
            ...
            MESSAGE_TYPE_BULK_INSERT => handle_bulk_insert(self, &message).await,
            MESSAGE_TYPE_UPSERT => handle_upsert(self, &message).await,
            ...
            MESSAGE_TYPE_SAVEPOINT => handle_savepoint(self, &message).await,
            MESSAGE_TYPE_RELEASE_SAVEPOINT => handle_release_savepoint(self, &message).await,
